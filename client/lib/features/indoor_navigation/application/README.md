...
- optimistic beam의 1등도 confirmed와 같은 간선 교체 관성을 쓴다. 다만 이것은 센서 후보
  안정화이고, 제품 마커의 마지막 안전선은 길안내 진행률이다. 모호하거나 확정 빔 재배치로
  lineage가 바뀐 preview는 보류 중인 진행률을 우회하지 못한다. 멀리 앞선 후보가 이후 정상
  peak를 내더라도 그 좌표로 한 번에 스냅하지 않고, **그 peak의 graph traversal 거리만큼**
  경로 위에서 따라잡는다. 코너 개수·각도·특정 에스컬레이터 ID는 조건에 없다. 검증 기준은
  `corridor_position_tracker_test.dart`, `route_progress_test.dart`,
  `indoor_guidance_session_test.dart`다.
- **방향 선택이 생기는 노드 앞뒤 약 3.5m는 회전 허용 구간이다.** 사람은 코너 안쪽을 잘라
  ...
  돌아가 실제 이탈을 숨기지 않는다.
- 활성 안내 경로는 **약한 prior**로만 쓴다. 전환 구간에서는 경로 순서상 정확한 다음 연결
  간선을 센서 방향이 아직 덜 지지해도 후보로 열고, 비경로 1등과 평균 오차가 2° 안인 사실상
  동점에서만 경로 쪽을 고른다. 더 큰 센서·위치 증거가 쌓이면 비경로 간선이 그대로 이긴다.
  따라서 코너 개수나 90° 각도를 특별 취급하지 않고, 실제 경로 이탈도 잠그지 않는다.
...
- **새 탑승 허가가 열린 뒤 평지인 첫 샘플**에서 탑승 고도 0점을 다시 잡는다. 오래된
  baseline이 기상 드리프트를 품은 채 허가와 함께 얼어, 실제로 4m 넘게 오른 뒤에야 후보가
  열렸던 2026-08-24 실측을 막는다. 이미 수직 속도가 문턱을 넘었으면 이 재설정은 보류한다.
- **하차 확정(`_confirm`)** 때 다시 잡는다. 그 순간의 평활 고도가 지금 서 있는 층의 고도다.
- **수직 이동이 시작된 뒤에는 절대 다시 잡지 않는다.** 화면은 반 층에서 목적 층으로 먼저 넘어가지만,
...
| 1차 수직 관측 | 수직 속도 0.12 m/s가 같은 방향으로 1초 지속 + 활성 경로 탑승 후보 | 현재 표시 위치와 최초 탑승·도착 노드 ID만 잠근다. 배너·걸음 pause·층 전환은 하지 않는다 |
| `verticalMotionDetected` | 1차 관측 **그리고** 안내가 지목한 에스컬레이터/탑승점 근접이면 실제 변위 0.5m, 노드 근거가 없으면 누적 고도 1.2m | 위치에 반영하는 걸음을 멈추고, 마커를 **반드시** 어딘가에 고정한다 |
...
**1차는 조용한 안전 고정이다.** 수직 속도가 같은 방향으로 1초 이어지고 활성 경로의 탑승
후보가 있으면, 그 순간 보이던 위치와 정확한 탑승·도착 노드 ID를 잠근다. 탑승 노드로
스냅하지 않고 배너·걸음 pause·층 전환도 하지 않는다. 이 고정은 0.5m를 기다리는 동안 발판
진동이 마커를 앞 복도나 반대 레인으로 밀고, 그 위치로 만든 새 경로가 최초 후보를 덮는
순서 경쟁만 차단한다. 경로 후보가 없으면 기존처럼 진단값만 선다.

...
/// 문 앞에서 **나침반을 믿지 않기로** 하는 어긋남(도).
///
/// 야외에서는 방향의 근거가 GPS 진행 방향이고 실내에서는 나침반이다. 문 하나를
/// 사이에 두고 근거가 통째로 바뀌는데, 그 순간의 나침반 값이 앵커 회전각으로
/// **구워져** 이후 실내 궤적 전체를 돌린다. 그래서 넘어가기 직전에 두 근거를 한
/// 번 맞춰 본다 — 방금 밖에서 걷던 방향과 나침반이 말하는 방향이 이만큼 넘게
/// 어긋나면, 그 나침반은 지금 이 자리에서 틀렸다고 본다.
///
/// 문 앞이 하필 자력계에 가장 나쁜 자리다: 철제 문틀·회전문·보안 게이트가 다
/// 그 자리에 있다. 세기는 정상 범위에 남고 방향만 도는 왜곡이라 기존 판정
/// ([isMagneticFieldPlausible]·[isHeadingErrorTrusted])이 못 잡는다.
///
/// 45°는 두 값 사이에 잡았다. 아래로는 마커 원뿔 반각(31°)과 course 자체의 오차를
/// 덮어야 하고, 위로는 "옆을 가리킨다"(90°)보다 낮아야 잡는 값이 있다.
///
/// **어긋났다고 course를 대신 쓰지는 않는다.** 진입 버튼은 멈춰 서서 누르므로 그
/// course는 이미 몇 초 전 값이고, 그사이 몸이 돌았을 수 있다. 나침반을 물리는
/// 근거로만 쓰고, 회전각은 그다음 사다리(문 앞 복도 축)가 정한다.
const double entryCourseDisagreementDeg = 45.0;

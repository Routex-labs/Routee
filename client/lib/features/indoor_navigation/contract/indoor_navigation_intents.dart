...
abstract interface class IndoorNavigationIntents {
  ...
  /// **안내를 시작하기 전까지** 걸음을 위치에 쌓지 않는다.
  ///
  /// 출발점을 직접 지정한 뒤 목적지를 고르고 `안내 시작`을 누르기까지, 사용자는
  /// 대개 그 자리에 서 있다. 그런데 걸음 판정은 제자리 흔들림도 걸음으로 세므로
  /// 아무도 걷지 않는데 마커가 지정한 자리에서 혼자 흘러갔다.
  ///
  /// **모아 뒀다 푸는 것이 아니다.** 붙드는 동안의 걸음은 그때그때 버린다(늦게
  /// 도착한 배치도 시간축으로 잘린다). 그래서 시작을 누른 순간 마커가 그동안의
  /// 몫만큼 한 번에 튀지 않고, 그 자리에서 첫 걸음부터 움직인다.
  ///
  /// **센서는 계속 흐른다** — 방위·기압·걸음 신호는 그대로 받는다. 멈추는 것은
  /// "그 걸음을 위치에 반영하는가" 하나뿐이다.
  ///
  /// [pauseStepTracking]과 **사유가 다르다. 둘은 서로를 풀지 않는다** — 탑승
  /// 중에 안내를 시작해도 걸음은 하차까지 멈춰 있어야 하고, 반대로 하차했다고
  /// 해서 아직 누르지 않은 `안내 시작`이 대신 눌리지도 않는다.
  Future<void> setStepsHeldBeforeStart({required bool held});

...
/// 진입 대조에 쓸 야외 진행 방향의 유효 기간.
///
/// 문 앞에 서서 「진입」을 누르기까지의 시간이다 — 카드를 읽고, 버튼을 찾고,
/// 누른다. 15초면 그 동작을 덮으면서도 "여기 서서 두리번거리다 들어간" 사람의
/// 몇 분 전 방향까지 끌고 오지는 않는다.
///
/// 이 값은 **나침반을 물리는 근거로만** 쓰인다([entryCourseDisagreementDeg]) —
/// 회전각으로 직접 쓰는 자리는 훨씬 짧게 잡는다(`_entryFloorDirection`).
///
/// 진입이 버튼으로 바뀐 뒤로 좌표 한 건에서 course를 다시 뽑는 갈래는 없앴다.
/// 사람은 멈춰 서서 누르므로 그 순간의 좌표에는 진행 방향이 아예 없다.
const entryCourseMemory = Duration(seconds: 15);

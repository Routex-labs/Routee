...
///
/// 실내 마커는 나침반(PDR 세션의 융합 heading)을 보지만, 야외에는 그 세션이
/// 돌지 않는다. 대신 좌표마다 딸려 오는 것이 **진행 방향**(course over ground)
/// 이다 — "폰이 어디를 향하고 있나"가 아니라 "직전까지 어느 쪽으로 움직였나"다.
///
/// 두 값의 뜻이 다르므로 그리는 규칙도 다르다. 여기서 하는 일은 셋이다.
///   1. 못 쓰는 값을 버린다(기기가 방향을 못 준 경우).
///   2. **서 있을 때는 그리지 않는다** — 움직이지 않으면 진행 방향이라는 것이
///      없고, 그때 오는 값은 좌표 잡음이 만든 각이다.
///   3. 방금까지 알던 방향은 잠깐 들고 있는다 — 신호등에 서거나 한 걸음 쉴
///      때마다 삼각형이 사라졌다 나타나면 깜빡이는 것으로만 보인다.
///

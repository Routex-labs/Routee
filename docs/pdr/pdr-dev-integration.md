...
해제되면 닫는다. PDR이 상시 실행이라 "시작~종료"를 경계로 쓸 수 없기 때문이다. 경로
단위로 끊으면 파일 하나가 v11 경로 지표와 같은 단위를 담는다.

**표본 배열에는 상한이 없다(v16).** 예전에는 품질 900 · tracker 입력 4000 · 경로진행
900 · 기압 3000 · 층 전이 200 · GPS 차이 900에서 오래된 것부터 버렸는데, 15분을 넘긴
주행에서는 출발 구간이 통째로 사라졌다 — 분석하려는 것이 바로 그 앞부분이다. 20분
주행 규모는 합쳐 6천 건 남짓, JSON 1 MB 안팎이다. 다시 상한이 필요해지면 **주행 시간 ×
표본 주기**로 잡는다(근거는 `PdrDebugSessionRecorder` 상수 자리 주석).

**디버그 모드에서는 건물을 나가도 세션이 안 닫힌다(v16).** 실내→실외→실내 한 주행을
JSON 하나로 남기기 위해서다. 원칙은 **"기록은 잇고 위치는 잇지 않는다"** — 앵커·궤적·
실내 경로는 나가는 순간 그대로 버리므로(`_dropIndoorPosition`) 재진입 때 위치는 새로
잡히고, 실외 구간은 `session_boundaries`의 `leftBuilding`~`reEntered` 사이로만 남는다.
디버그가 꺼진 일반 사용자에게는 예전 동작 그대로다(나가면 PDR 세션이 멈춘다).
검증: `client/test/screens/outdoor_map/debug_session_spans_building_exit_test.dart`,
`client/test/features/indoor_navigation/pdr_debug_unbounded_session_test.dart`.

**안내가 끝나도(`routeEnded`) 세션은 안 닫힌다 — 닫는 것은 내보내기 하나뿐이다.**
경계는 그대로 찍히지만 레코더를 갈지 않는다(`_beginRouteRecordingSession`). 안내는
출구에서 끝나는데 실측에서 보려던 것 — 문 밖 GPS 표류, 길 건너에서 시작하는 경로,
재진입 뒤 마커 — 은 전부 그 **다음에** 일어나서, 예전에는 밖에서 새 길을 잡는 순간
레코더가 갈려 매번 절반에서 잘렸다. 대신 표본 배열에 상한이 없으므로 **끝은 사람이
정한다**: 공유 버튼을 누르면 `exported` 경계를 찍고 그 파일을 넘긴 뒤 곧바로 새 세션을
연다(그 사실은 스낵 한 줄로 말한다). 내보낸 뒤 계속 걸으면 그 구간은 다음 파일에 남는다.
디버그가 꺼져 있으면 예전대로 **길안내 한 건이 세션 하나**다.
검증: `client/test/screens/outdoor_map/debug_session_survives_route_end_test.dart`.

**파일 크기는 주행 시간에 정비례한다**(상한이 없으므로). 20분 ≈ 1 MB가 실측 기준이라
1시간 ≈ 3 MB, 2시간 ≈ 6 MB로 잡는다. 공유는 임시 파일 URI로 넘어가므로(share_plus가
바이트를 캐시에 쓴 뒤 경로를 넘긴다) 이 크기는 공유 시트에서 문제가 되지 않는다 —
걸리는 쪽이 있다면 받는 앱의 첨부 한도다.

**`indoor_state_changes`가 0건인 것은 대개 정상이다.** 이 배열은 `_setIndoorEntered`가
값이 **실제로 바뀔 때만** 남긴다. 레코더는 길안내가 시작될 때 열리는데 그때는 이미
실내라, 진입 전이(false→true)는 레코더보다 **먼저** 일어나 파일에 안 들어온다. 실내
상태의 시계열이 필요하면 `gps_position_deltas`를 본다 — GPS 좌표마다
`building_verdict`와 `indoor_entered`가 함께 있어 판정과 상태를 둘 다 되짚을 수 있다.
그래서 판정 자체를 따로 남기지 않는다(같은 사실을 두 배열에 적는 것이 된다).
...
| v15 | 실내 이탈 이벤트(약한·확정), GPS와 화면 위치의 거리 시계열, 층별 궤적 | 이탈이 걸린 시각이 파일에 없어 "문을 지난 뒤 궤적이 어떻게 흘렀는지"를 못 봤다. GPS가 얼마나 멀리 찍혔는지도 잴 값이 없었다. 그리고 `paths`가 **현재 층 하나만** 그려, 층을 오간 세션은 이벤트 배열만 전 구간이고 궤적은 마지막 층만 남았다 |
| v16 | 세션 경계 **시계열**(`session_boundaries`)과 `spans_building_exit`, 실내 상태 변경의 **출처**(`indoor_state_changes`) | 경계가 문자열 하나라 마지막 값만 남았고, 실내→실외→실내 한 세션에서 **어디가 실외 구간인지** 자를 수 없었다. 그리고 `indoor_entered`가 "도면을 보고 있다"와 "건물 안에 있다"를 겸해서, 확대만으로 켜진 실내 상태와 GPS가 판정한 실내 상태를 파일에서 못 가렸다 |
| v17 | anchor의 `magnetic_declination_deg`와 `debug_heading_offset_deg` | `rotation_deg` 하나만 남아서, 헤딩이 돌아간 세션을 봐도 **자편각 상수 때문인지 현장에서 돌린 노브 때문인지** 가를 수 없었다. 상수를 굳히려면 두 값이 따로 남아야 한다 |
| v18 | 문 근거가 **안 걸린** 이유와 그때의 최단 거리(`exit_door_closest_miss`), GPS 좌표별 `meters_outside` | 이탈이 아예 안 된 주행에서 파일에 **아무 흔적도 없었다.** 문 근거는 걸렸을 때만 남아서, 안 걸린 주행은 문턱 15 m가 문제인지 PDR이 문까지 못 온 것인지 가릴 값이 없었다. `unclear` 구간도 판정 이름만 있고 **얼마나 바깥이었는지**가 없어, 약한 이탈의 거리·지속 문턱을 되짚을 수 없었다 |
| v19 | 도약으로 **거른** GPS 좌표(`gps_rejected_fixes`) — 도약 거리·허용치·경과 시간·그때의 보고 정확도·항복 여부 | 출구 앞에서 수신기가 오차 11~13 m를 보고하면서 실제로는 50~77 m를 튀었다. 거르기를 넣어도 파일에는 **거른 흔적이 없어**, 규칙이 맞게 물었는지·너무 빡빡해 항복만 하는지 다음 실측에서 가릴 값이 없다 |
| v20 | 진입 시각 ~ 첫 실내 위치 사이의 공백(`indoor_position_gaps`) | 진입 직후 화면에 **위치 마커가 하나도 없는** 구간이 있었는데(GPS 마커는 진입과 동시에 꺼지고 실내 마커는 앵커가 없어 안 뜬다), 그 길이를 재려면 `gps_position_deltas`에서 `position_source`가 null인 줄을 눈으로 세어야 했다. 25초가 줄었는지 다음 실측에서 숫자 하나로 보려고 넣었다 |
| v21 | `actual_marker_position`을 진행률 보호·탑승 고정까지 적용된 제품 마커 좌표로 교정 | 이 필드가 `preview_position`의 별칭이라, 로그상 9 m 튄 preview와 실제 화면에서 붙든 마커를 구분할 수 없었다. 디버그 모드가 그리기 동작을 바꾼 것처럼 보인 원인도 같은 파일 안에서 가릴 수 없었다 |

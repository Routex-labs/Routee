...
### 100.7 µT의 출처는 아직 안 갈렸다

셋 중 하나이고, 앱이 할 일은 어느 쪽이든 같다(못 믿는 방위를 안 쓰는 것).

1. **자력계가 보정된 적이 없다.** 하드아이언 오프셋이 남으면 세기·복각이 딱 이렇게
   나온다. 안드로이드가 정확도를 `unknown`으로만 주는 것도 이와 맞는다.
2. **책상 위 자석**(노트북·거치대·무선충전기).
3. **폰 자체.** 플립은 접힘용 자석이 센서 근처에 있다.

가르는 법은 8자 흔들기 → 다시 측정이다. 세기가 50 근처로, 복각이 53 근처로
내려오면 1번이다. 그대로면 2·3번이고, 폰을 책상에서 1 m 떼어 다시 재면 갈린다.

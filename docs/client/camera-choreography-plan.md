...
## 5. 문 경유 여정 — 카메라의 주인이 경로가 아니다

야외에서 건물 안 매장으로 길을 찾는 여정은 이 문서의 다른 연출과 전제가 다르다.
**그려지는 야외 경로가 사용자가 가려는 곳이 아니다** — 그 선은 지상 출입구에서 끝나고,
목적지는 건물 안에 있다. 그래서 경로 전체에 카메라를 맞추면 화면이 건물 앞에 서고,
정작 사용자가 고른 매장은 도면 밖에 남는다.

### 계획 화면은 목적지 매장, 안내 시작은 출발지

| 순간 | 카메라 | 왜 |
|---|---|---|
| 목적지 확정 | 목적지 **매장**(검색에서 그 매장을 눌렀을 때와 같은 `focusStore`) | 길찾기를 열기 직전에 보던 그림이 그것이다. 무엇을 고른 것인지 다시 확인할 자리가 여기뿐이다 |
| `안내 시작` | 출발지(그다음부터 GPS 팔로우) | 지금부터 걸을 구간의 시작점이다 |
| 실내 구간을 못 푼 갈래 | 경로 전체(예전 그대로) | 안내가 정말 문 앞에서 끝나므로 그 구간이 곧 여정이다 |

활성 층도 같은 시각에 갈린다 — 규칙과 표는
[실내 진입·이탈 규칙](indoor-entry-rules.md) 7절.

### 개요 fit을 **한 번** 건너뛴다

경로가 그려지는 순간 `_applyRoute`가 경로 전체로 물러선다. 그대로 두고 매장 포커스를
이어 붙이면 화면이 경로 전체로 갔다가 매장으로 돌아온다 — [4.14](#414-카메라-주인이-겹치는-자리는-시간으로-갈린다)가
말하는 겹침 그대로다. 그래서 `_skipRouteCameraFitOnce`로 그 한 번만 비켜 준다.
**한 번 쓰고 내린다** — 남겨 두면 다음 안내의 개요가 조용히 사라지고, 그것은 화면만
봐서는 원인을 짚을 수 없는 종류의 버그다.

### 축소 연출을 따로 만들지 않는다 — flyTo가 이미 그린다

"확대된 매장 화면에서 물러섰다가 출발지로 내려앉는" 그림은 **연출이 아니라 API의
성질**이다. maplibre_gl의 `animateCamera`는 시간이 붙으면 양쪽 플랫폼 모두 flyTo로
내려간다(Android `MapLibreMap.animateCamera`, iOS `MLNMapView.fly(to:withDuration:)`).
flyTo는 먼 자리로 갈수록 중간에 더 크게 축소되는 호를 그리므로, 목표를 하나만 주면 된다.

**버린 대안: 축소와 이동을 두 애니메이션으로 쪼개기.** 직관적이지만 4.14가 잡은 겹침을
스스로 만드는 길이다. 앞 애니메이션이 끝나기 전에 뒤엣것이 걸리면 화면이 떨고, 기다리면
시작을 누른 순간과 화면이 움직이는 순간이 벌어진다. 그림이 밋밋하면 손잡이는
`guidanceStartRecenterDuration`(700ms) 하나다 — 시간을 빼거나 `moveCamera`로 바꾸면
호 자체가 사라져 화면이 순간이동한다.
